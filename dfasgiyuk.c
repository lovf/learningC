...
int main () {
	char str1[] = "hello";
	char str2[] = "hello";
	if (str1 == str2) { //数组名不能直接进行比较大小,但是指针可以进行比较大小,数组会隐式转化为指针,指向首元素的地址
		printf("相等\n");
	}
	else  {
		printf("不相等\n");
...
由于上述str1和str2属于两个不同的数组,因此二者的地址是不相同的. == 在c语言比较的是地址/身份,而strcmp(str1,str2)进行进行比较字符串的内容,也能比较字符串的大小(字典序)
#include <stdio.h>
#include <stdlib.h>
...
int main () {
	...
	char* str1 = "hello";
	char* str2 = "hello";
	if (str1 == str2) { // 此时的str1和str2分别代表字符串"hello"的地址,而同一个hello对应相同的地址,因此进行比较时二者是相等的.
		printf("相等\n");
	}
	else {
		printf("不相等\n");
