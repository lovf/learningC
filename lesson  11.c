...
//在函数的调用过程中,假如含有参数那么在运行的过程进行传参时,一定进行传指针,否则没有进行本质的变化.
///////////////////
...
#include <stdio.h>
#include <stdlib.h>
//#include "lesson.h" 类似于将声明包含在头文件lesson.h中
//对于一般情况来说，自定义利用双引号进行包含,标准库中利用<>进行包含

//函数的声明
extern   Add(int x, int y);
extern  Sub(int x,int y);
extern Mul(int x,int y);
extern Div(int x,int y);
...
////函数调用的执行过程: 函数调用就会进入到函数体内部执行,遇到return语句就结束,回到函数调用的位置继续执行
//递归:程序调用自身的编程技巧
